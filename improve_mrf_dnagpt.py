...
def get_data(feature1, feature2):
    ...
    for k, (data1, data2) in enumerate(zip(feature2, feature1)):
        for i in range(10):
            temp = data1[i][:, 0]
...
class GCN(nn.Module):
    def __init__(self, device, t, filter_pos):
        ...
        self.w_final = nn.Parameter(torch.randn(1) * 0.00001)
    ...
    def get_mean_norm(self, A, M):
        masked_A = A * M
        output = torch.sum(masked_A, dim=2) / (torch.sum(M, dim=2) + 1e-10)
        output = torch.mean(output, dim=1)
        return output


for filter_pos, t in zip([0, 30], [0, 10]):
    for method in ['DNAGPT']:
        for dataset in [args.dataset]:
            # for dataset in ['Essay', 'Reuters', 'DetectRL']:
            # for dataset in ["dipper", 'polish', 'back_translation']:
            ...
            for LLM in LLMs:
                for iter in range(5):
                    ...
                    print(LLM, iter)
                    print('datasets/embedding/x_train_lprob_%s_%s_%s_-1_%d.pkl' % (ref_dataset, LLM, method, iter))

                    with open('datasets/embedding/x_train_lprob_%s_%s_%s_-1_%d.pkl' % (
                            ref_dataset, LLM, method, iter),
                              'rb') as f:
                        feature1 = pkl.load(f)
                    with open(
                            'datasets/embedding/x_train_lprob_regens_%s_%s_%s_-1_%d.pkl' % (
                                    ref_dataset, LLM, method, iter),
                            'rb') as f:
                        feature2 = pkl.load(f)
                    with open(
                            'datasets/embedding/y_train_%s_%s_%s_-1_%d.pkl' % (ref_dataset, LLM, method, iter),
                            'rb') as f:
                        train_y = np.array(pkl.load(f))
                    train_x = get_data(feature1, feature2)

                    with open(
                            'datasets/embedding/x_val_lprob_%s_%s_%s_-1_%d.pkl' % (ref_dataset, LLM, method, iter),
                            'rb') as f:
                        feature1 = pkl.load(f)
                    with open(
                            'datasets/embedding/x_val_lprob_regens_%s_%s_%s_-1_%d.pkl' % (
                                    ref_dataset, LLM, method, iter),
                            'rb') as f:
                        feature2 = pkl.load(f)
                    with open(
                            'datasets/embedding/y_val_%s_%s_%s_-1_%d.pkl' % (ref_dataset, LLM, method, iter),
                            'rb') as f:
                        val_y = np.array(pkl.load(f))
                    ...
                    optimizer = optim.AdamW(model.parameters(), lr=lr_map[method], weight_decay=10.)
                    ...
                    for llm in llms:
                        with open(
                                'datasets/embedding/x_test_lprob_%s_%s_%s_-1_%d.pkl' % (
                                        ttest_dataset, llm, method, iter),
                                'rb') as f:
                            feature1 = pkl.load(f)
                        with open(
                                'datasets/embedding/x_test_lprob_regens_%s_%s_%s_-1_%d.pkl' % (
                                        ttest_dataset, LLM, method, iter),
                                'rb') as f:
                            feature2 = pkl.load(f)
                        with open(
            ...
            for detectLLM in LLMs:
                with open('logs/temp_%s_%s_%s_%d_%d.txt' % (dataset, method, detectLLM, filter_pos, t), 'w') as ff:
                    for llm in llms:
                        for iter in range(5):
