...
def cur_evaluation(model, cur_dataset, data_loader):
    ...
    with torch.no_grad():
        for x, m, y in pbar:
            x = x.to(device)
            m = m.to(device)
            adj_in = torch.tensor(train_dataset.adj_in).to(device)
...
class GCN(nn.Module):
    def __init__(self, device, t, filter_pos):
        ...
        self.weights = torch.tensor([1 / (1 + np.exp(-(i - filter_pos))) for i in range(1023)], dtype=torch.float32).to(
            device)
        self.to(device)
    ...
    def forward(self, x, adj_in, m):
        ...
        for i in range(self.t):
            x2 = torch.matmul(
                torch.sparse.mm(adj_in, H.permute(1, 2, 0).reshape((1023, -1))).reshape((1023, 2, -1)).permute(2, 0,
                                                                                                               1),
                self.w_in)
            H = torch.softmax(torch.log(H + 1e-10) - x2, dim=-1)

        if (self.filter_pos > 0):
            x_tilde = self.mean(x[:, 1:, :, 1], m[:, 1:, :], self.weights[None, None, :])
            x = self.mean(H[:, :, 1], m[:, 0, :], self.weights[None, :])
        else:
            x_tilde = self.mean_norm(x[:, 1:, :, 1], m[:, 1:, :])
            x = self.mean_norm(H[:, :, 1], m[:, 0, :])

        mean = x_tilde.mean(dim=-1)
        var = x_tilde.std(dim=-1)
    ...
    def mean(self, A, M, weights):
        res = torch.sum(A * M * weights, dim=-1) / (torch.sum(M * weights, dim=-1) + 1e-10)
        return res

    def mean_norm(self, A, M):
        res = torch.sum(A * M, dim=-1) / (torch.sum(M, dim=-1) + 1e-10)
        return res


for filter_pos, t in zip([0, 30], [0, 10]):
    for method in ['FastGPT']:

        for dataset in [args.dataset]:
            # for dataset in ['Essay', 'Reuters', 'DetectRL']:
            if (dataset in ['Mix', "dipper", 'polish', 'back_translation', 'DetectRL']):
            ...
            for LLM in LLMs:
                for iter in range(5):
                    ...
                    x_likelihood = np.load(
                        'datasets/embedding/x_likelihood_train_%s_%s_%s_%d_%d.npy' % (
                            ref_dataset, LLM, 'FastGPT', -1, iter))
                    x_likelihood_tilde = np.load(
                        'datasets/embedding/x_likelihood_tilde_train_%s_%s_%s_%d_%d.npy' % (
                            ref_dataset, LLM, 'FastGPT', -1, iter))
                    temp_x = np.swapaxes(
                        np.concatenate([x_likelihood, x_likelihood_tilde], axis=-1, dtype=np.float32),
                        1,
                        2)
                    train_x = np.zeros_like(temp_x, dtype=np.float32)
                    train_x[:, :, :len(temp_x[0, 0])] = temp_x

                    train_y = np.load(
                        'datasets/embedding/y_train_%s_%s_%s_%d_%d.npy' % (ref_dataset, LLM, 'FastGPT', -1, iter))

                    x_likelihood = np.load(
                        'datasets/embedding/x_likelihood_val_%s_%s_%s_%d_%d.npy' % (
                            ref_dataset, LLM, 'FastGPT', -1, iter))
                    x_likelihood_tilde = np.load(
                        'datasets/embedding/x_likelihood_tilde_val_%s_%s_%s_%d_%d.npy' % (
                            ref_dataset, LLM, 'FastGPT', -1, iter))
                    temp_x = np.swapaxes(
                        np.concatenate([x_likelihood, x_likelihood_tilde], axis=-1, dtype=np.float32),
                        1,
                        2)
                    val_x = np.zeros_like(temp_x, dtype=np.float32)
                    val_x[:, :, :len(temp_x[0, 0])] = temp_x
                    val_y = np.load(
                        'datasets/embedding/y_val_%s_%s_%s_%d_%d.npy' % (ref_dataset, LLM, 'FastGPT', -1, iter))
                    ...
                    for llm in llms:
                        x_likelihood = np.load(
                            'datasets/embedding/x_likelihood_test_%s_%s_%s_%d_%d.npy' % (
                                ttest_dataset, llm, 'FastGPT', -1, iter))
                        x_likelihood_tilde = np.load(
                            'datasets/embedding/x_likelihood_tilde_test_%s_%s_%s_%d_%d.npy' % (
                                ttest_dataset, llm, 'FastGPT', -1, iter))
                        temp_x = np.swapaxes(
                            np.concatenate([x_likelihood, x_likelihood_tilde], axis=-1, dtype=np.float32),
                            1, 2)
                        test_x = np.zeros_like(temp_x, dtype=np.float32)
                        test_x[:, :, :len(temp_x[0, 0])] = temp_x

                        test_y = np.load(
                            'datasets/embedding/y_test_%s_%s_%s_%d_%d.npy' % (
                                ref_dataset, llm, 'FastGPT', -1, iter))
