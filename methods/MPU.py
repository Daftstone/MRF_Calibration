...
class MPU(nn.Module):
    def __init__(self, DEVICE, save_path, load_path, pos_bit=1):
        ...
        self.fc = nn.Sequential(
            nn.Linear(768 * self.max_token * args.sentence_num, 256),
            nn.ReLU(),
            nn.Linear(256, 64),
            nn.ReLU(),
            nn.Linear(64, 2),
        ...
        with torch.no_grad():
            ...
            for i in range(0, index_list.shape[0], num_sentences):
                ...
                mean_embeddings = (word_embedding[mins:maxs] * mask[mins:maxs]).view(
                    768 * self.max_token * args.sentence_num)
        ...
        if (args.reg == 0):
            ...
            try:
                ...
                for i in range(0, index_list.shape[0], num_sentences):
                    mins = i
                    maxs = i + num_sentences
                    # mean_embeddings = torch.sum(word_embedding[mins:maxs] * mask[mins:maxs], dim=0)
                    mean_embeddings = (word_embedding[mins:maxs] * mask[mins:maxs]).view(
                        768 * self.max_token * args.sentence_num)
                    combined_embeddings.append(mean_embeddings)
                combined_embeddings = torch.stack(combined_embeddings)  # (batch_size/3, seq_len*4, hidden_size)

                # para_logits = self.sent_detector(inputs_embeds=combined_embeddings).logits
