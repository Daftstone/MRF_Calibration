...
def get_mask(x):
    ...
    for i in range(len(mask)):
        ...
        if (len(idx) == 0 or np.any(x[i][idx[-1]:] != 0)):
            mask[i] = 1
        else:
            for iidx in idx:
                if (np.all(x[i][iidx:] == 0)):
                    mask[i, :iidx] = 1
                    break
    return mask
...
class my_dataset(Dataset):
    def __init__(self, data, labels, filter_pos):
        ...
        self.adj_in = np.zeros((len(data[0]), len(data[0])), dtype=np.float32)
        if (filter_pos > 0):
...
class GCN(nn.Module):
    def __init__(self, device, t, filter_pos):
        ...
        self.weights = torch.tensor([1 / (1 + np.exp(-(i - self.filter_pos))) for i in range(1024)],
                                    dtype=torch.float32).to(device)[None, :]
        self.to(device)
    ...
    def forward(self, x, adj_in, m):
        ...
        for i in range(self.t):
            x2 = torch.matmul(
                torch.sparse.mm(adj_in, x.permute(1, 2, 0).reshape((1024, -1))).reshape((1024, 2, -1)).permute(2, 0,
                                                                                                               1),
                self.w_in)
            x = torch.softmax(torch.log(x + 1e-10) - x2, dim=-1)

        x = x[:, :, 1]
        if (self.filter_pos > 0):
            x = torch.sum(x * m * self.weights, dim=1) / (torch.sum(m * self.weights, dim=1) + 1e-10)
        else:
            x = torch.sum(x * m, dim=1) / (torch.sum(m, dim=1) + 1e-10)
...
for filter_pos, t in zip([0, 30], [0, 10]):
    for method in [args.method]:
        for dataset in [args.dataset]:
            ...
            lr_map = {'Log_Rank': 0.05, 'Log_Likelihood': 0.05, 'Entropy': 0.05}
            ...
            for LLM in LLMs:
                for iter in range(5):
                    ...
                    print(LLM, iter)
                    temp_x = np.load(
                        "datasets/embedding/x_train_%s_%s_%s_-1_%d.npy" % (ref_dataset, LLM, method, iter))
                    train_x = np.zeros((len(temp_x), 1024), np.float32)
                    train_x[:, :len(temp_x[0])] = temp_x
                    train_y = np.load(
                        "datasets/embedding/y_train_%s_%s_%s_-1_%d.npy" % (ref_dataset, LLM, method, iter))

                    temp_x = np.load(
                        "datasets/embedding/x_val_%s_%s_%s_-1_%d.npy" % (ref_dataset, LLM, method, iter))
                    val_x = np.zeros((len(temp_x), 1024), np.float32)
                    val_x[:, :len(temp_x[0])] = temp_x
                    val_y = np.load(
                        "datasets/embedding/y_val_%s_%s_%s_-1_%d.npy" % (ref_dataset, LLM, method, iter))
                    ...
                    for llm in llms:
                        temp_x = np.load(
                            "datasets/embedding/x_test_%s_%s_%s_-1_%d.npy" % (ttest_dataset, llm, method, iter))
                        test_x = np.zeros((len(temp_x), 1024), np.float32)
                        test_x[:, :len(temp_x[0])] = temp_x
                        test_y = np.load(
                            "datasets/embedding/y_test_%s_%s_%s_-1_%d.npy" % (ttest_dataset, llm, method, iter))
