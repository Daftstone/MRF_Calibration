...
def get_sampling_discrepancy_analytic(logits_ref, logits_score, labels):
    ...
    return discrepancy.item(), log_likelihood.cpu().data.numpy(), mean_ref.cpu().data.numpy(), var_ref.cpu().data.numpy()
...
def get_text_crit(text, args, model_config):
    ...
    with torch.no_grad():
        ...
        text_crit, logs, means, vars = get_sampling_discrepancy_analytic(logits_ref, logits_score, labels)

    return text_crit, logs, means, vars
...
def run_fast_experiments(data, base_model, base_tokenizer, score_model, score_tokenizer):
    ...
    for nn in ['train', 'test', 'val']:
        ...
        for text in tqdm(test_text):
            text_crit, log, mean, var = get_text_crit(text, args, model_config)
            if (np.isnan(text_crit)):
                test_preds.append(0.)
            else:
                test_preds.append(text_crit)
            if (len(log[0]) == 1023):
                log_list.append(log)
                mean_list.append(mean)
                var_list.append(var)
            else:
                log_temp = np.zeros((1, 1023))
                mean_temp = np.zeros((1, 1023))
                var_temp = np.zeros((1, 1023))
                log_temp[:, :len(log[0])] = log
                mean_temp[:, :len(log[0])] = mean
                var_temp[:, :len(log[0])] = var
                log_list.append(log_temp)
                mean_list.append(mean_temp)
                var_list.append(var_temp)

        log_list = np.concatenate(log_list, axis=0)
        mean_list = np.concatenate(mean_list, axis=0)
        var_list = np.concatenate(var_list, axis=0)

        np.save("datasets/embedding/x_log_%s_%s_%s_%s_%d_%d.npy" % (nn,
                                                                    args.dataset, args.detectLLM, args.method,
                                                                    args.sentence_length, args.iter), log_list)
        np.save("datasets/embedding/x_mean_%s_%s_%s_%s_%d_%d.npy" % (nn,
                                                                     args.dataset, args.detectLLM, args.method,
                                                                     args.sentence_length, args.iter), mean_list)
        np.save("datasets/embedding/x_var_%s_%s_%s_%s_%d_%d.npy" % (nn,
                                                                    args.dataset, args.detectLLM, args.method,
                                                                    args.sentence_length, args.iter), var_list)
    ...
    acc_test, auc_test, tpr1_test, tpr2_test, tpr3_test = test_res
    print(
        f"{'Fast'} acc_test: {acc_test}, auc_test: {auc_test}, tpr1_test: {tpr1_test}, tpr2_test: {tpr2_test}, tpr3_test: {tpr3_test}")
